# Is there a Postgres to test the db backend's *other* claim path
# against?
#
# void/jobs-db has two of them, and they are different code: on
# Postgres a claim is one `UPDATE ... WHERE id = (SELECT ... FOR
# UPDATE SKIP LOCKED) RETURNING`, and everywhere else it is a select
# and a re-checked update inside a transaction. test/db-test.janet
# runs the conformance suite over the portable path (sqlite, which has
# no SKIP LOCKED); this file is how the Postgres one gets run at all.
#
...
#
# CI sets it against a service container, so the SKIP LOCKED path is a
# real gate there. On a laptop without one the suite announces itself
# skipped rather than failing — the same bargain void/db-postgres and
# void/redis strike, for the same reason.
#
...
# make `cd jobs && jpm test` fail to *compile* on a machine that has
# not built it — on the suite that needs no database at all.
