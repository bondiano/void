...
###
### Every flow driven the way a browser drives it: test/inject runs a
### request through the whole chain — the session, the CSRF check, the
### strategy chain, the route table — without opening a socket, so what
### passes here is what a browser gets.
###
...
### refuses a challenge nobody delivered; in production :void/mail-auth is
### what delivers, and here it is five lines that keep the code where an
### assertion can read it — which is how the reset and the verification
### are proven without a mailbox.
...
(test/with-http [c {:plugins plugins
  ...
  # -- CSRF --------------------------------------------------------------
  #
  # The credential rides on a cookie, which is exactly when CSRF applies.
