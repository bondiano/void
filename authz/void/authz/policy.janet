### void/authz/policy — the registry of named policies.
###
...
### same decision made twice. What makes the system inspectable is the
### **registry**, not the shape of the body: `(authz/policies)` lists what
### exists, `void authz policies` prints it, and route metadata refers to
### a policy by keyword, so the route table shows where each one is
### enforced.
###
...
###
### A policy answers `true`, `false`, or a **string** — the string is the
...
(defmacro defpolicy
  ...
  Sugar over `register!`: the macro adds nothing the data form cannot
