### void/bench/targets — the bench-suite as data.
###
### Three tables: `benches` are the load shapes (what the generator does),
### `targets` are the servers (what it hits — the B* mini-apps and the
### contextual baselines), `budgets` are the SLOs. The runner is a pure
### interpreter of these tables; adding B2-B4 in later waves means adding
### rows, not code.

(def benches
  ``Load shapes. :rate is wrk2's fixed RPS for the latency runs —
  80% of the throughput floor, never max throughput (latency at saturation
  is meaningless). :script/:body-file are bench-root relative; the lua
  script posts the payload named by BENCH_BODY_FILE.``
  {:plaintext {:path "/"
               :threads 4
               :connections 64
               :rate 16000}
   ...
   # instead. `:connections` is the budgeted thousand and `:rate` the
...
(def budgets
  "The budgets (1 worker, 1 vCPU), verified against the recorded
  ...
  {:b0 {:p50 2 :p99 3 :rps 20000 :loop-lag-p99 1}
   ...
   # B2/B3 are hypotheses until this wave measures them on the reference
   # environment; B3 additionally carries the GC budget, and carries it as
   # :loop-lag-max, because a stop-the-world pause on a single-threaded
   # loop is loop lag of at least its own length and janet 1.41 reports no
   ...
   # B4 has a kind of its own because it has metrics of its own: gives it
   # no p50/p99 and no request throughput, only "delivery < 50 ms" to 1000
   # connections at 10k msg/s. `:rps` here counts messages delivered to
   # peers per second.
   #
   # It deliberately carries no :loop-lag-p99, and that is an argument
   # rather than an omission. The "p99 < 1 ms under target load" budget is
   # a budget about a loop that is *waiting* between units of work: a
   # request arrives, is served, and the lag measures how late the loop
   # was to notice. A broadcast is the opposite shape — the target load is
   # one burst of a thousand writes per tick, so the lag during a tick
   # *is* the fan-out, and a 1 ms p99 would mean the whole fan-out
   # finished inside a millisecond. What that budget is a proxy for — is
   # the loop keeping up? — is measured directly here by the delivery
   # percentile. The loop-lag numbers are still read from the probe and
   # printed with the row; they are just not a gate on this shape.
...
(def targets
  ...
  Baselines (:baseline true) calibrate the class — Go net/http is the
  ceiling, FastAPI+uvicorn the Python class — and are never checked
  against budgets.
  ...
  {:b0 {:doc "B0 plaintext hello — void router + full middleware stack"
   ...
   :b1-pressure {:doc "B1 with void/pressure in the chain — the bench row every middleware ships"
                 :bench :json
                 :port 8101
                 # the same app, one plugin heavier: `void bench b1
                 # b1-pressure` prints both rows and the delta is the
                 # answer every middleware author owes. No budget of its
                 # own — B1's budget is B1's
                 :cmd "VOID_BENCH_PRESSURE=1 exec janet apps/b1-json-echo/main.janet"}
   :b1-obs {:doc "B1 with void/obs in the chain — the instrumentation-overhead row"
            :bench :json
            :port 8101
            # the same app, two plugins heavier: `void bench b1 b1-obs`
            # prints both rows, and the delta is what is budgeted at ≤ 7%
            # of throughput. No budget of its own — B1's is B1's, and obs
            # is measured against B1, not against the SLO
...
(def default-targets
  ``What a bare `void bench` runs: every budgeted B* row. B2/B3
