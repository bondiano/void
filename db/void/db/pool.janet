...
(defn- open-entry
  {:params [DbNormalizedDriver :number]
   :ret DbPoolEntry
   :throws [VoidError]}
...
(defn driver-of
  {:params [DbPool] :ret DbNormalizedDriver}
  "The driver a pool runs on."
  [pool]
  (pool :driver))

(defn note-query!
  {:params [DbPool :number] :ret :nil}
...
(defn checkin
  {:params [DbPool DbPoolEntry] :ret :nil}
