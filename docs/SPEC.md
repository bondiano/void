...
- Content negotiation, static files (etag, range), cookies (`Cookie` и `Set-Cookie` разбираются и собираются в обе стороны — `wire/cookie-header`, `wire/parse-set-cookie`, `ring/cookie-str`), multipart в обе стороны (`multipart/parse` и `multipart/encode` — один формат, одна реализация для сервера и клиента).
...
- **Client** *(волна 4, ADR-0027)*: `void/http/client` — HTTP/1.1 наружу теми же парсерами `void/http/wire`, keep-alive на одно соединение, таймаут отдельной задачей с `ev/deadline`. Запрос — данные: любой метод, `:query` парами, `:cookies` именами и значениями, тело как `:body`/`:form`/`:multipart` (энкодеры — те же, которыми сервер читает). Ответ — данные: `header`/`header-values`, `cookies` с атрибутами (`wire/parse-set-cookie`), `redirect?`/`follow-target`; редиректы следуются только по просьбе (`:follow n`) и роняют `Authorization`/`Cookie` при смене origin. Без TLS (ADR-0010), без cookie jar (обе половины разобраны — политика вызывающего), без ретраев кроме переоткрытия keep-alive-сокета, закрытого peer'ом. Исходящий trace-контекст кладёт вызывающий (`trace/inject!`), счётчики читает `void/obs` через `client/stats`.
...
### 5.13 `void/obs` — observability *(L, Spring-level это много; ADR-0021)*
- Три plugin'а: `void/obs` (реестр, трейсер, runtime-сэмплер, логи, инструментации — только core, годится воркеру, CLI и prefork-мастеру), `void/obs-http` (RED, корневой спан, queue time, эндпоинты — единственное, чему нужен `void/http`) и `void/obs-otlp` (экспорт спанов и метрик в коллектор — волна 4, ADR-0027). Тот же шов, что у `void/cache` / `void/cache-http`.
- **Logs**: ядро — `void/core/log` (§3.7, ADR-0018, доступно с волны 1); obs добавляет trace-id/span-id в log-context (их кладёт `with-span` — шов, оставленный ADR-0018), sampling **воротами перед всем списком sinks** (уровень выше порога — всегда; запись внутри сэмплированного спана — всегда; иначе ставка) и файловые sinks (своя фибра за буферизованным каналом, переполнение — потеря со счётчиком, `:fatal` синхронно). OTLP-log-sink не сделан и в волне 4 (HTTP-клиент для него уже есть — ADR-0027 §7): каждая запись, доехавшая до файла, доезжает до коллектора через агента, который этот файл и так читает.
- **Metrics**: counters/gauges/histograms прометеевской модели; метрика объявляется модулем-владельцем на загрузке и возвращает handle, запись — поиск по таблице кортежа меток. **Кардинальность ограничена по построению**: `:max-label-sets` на метрику, отказ считается в `:void.obs/metrics-dropped-total`. Gauge может быть pull-based (`:collect` в момент скрейпа) — так в экспозицию попадают пулы, RSS и in-flight. RED на каждый route — по **имени маршрута** (`:void.obs/name`, иначе `:name`), никогда по пути запроса. Runtime: loop-lag, RSS, uptime, in-flight, соединения, потерянные лог-записи; **GC- и fiber-метрик нет** — janet 1.41 не сообщает ни пауз, ни аллокаций, ни числа фиберов, а граница сверху на паузу GC видна как максимум loop-lag (тот же аргумент, что у `void/bench/probe`). Экспорт: Prometheus text как чистая проекция снапшота; OTLP/JSON (`void/obs-otlp`, ADR-0027) — вторая проекция того же снапшота, cumulative и под теми же именами серий; protobuf-кодировка — после `void/proto`.
- **Tracing**: span API, контекст в dyn (per-fiber — изоляция бесплатно), W3C traceparent propagation (in/out), экспортёры — точка `:void.obs/exporter`; OTLP export — вклад в неё из `void/obs-otlp` (волна 4, ADR-0027): ограниченная очередь, батч, потери со счётчиком. Корневой спан HTTP-запроса создаётся, **только когда его есть кому прочитать** (настроен экспортёр, пришёл `traceparent` или попрошено `[:obs :trace :always]`) — так закрывается бюджет §8.2 (ADR-0021).
- Auto-instrumentation через component-декораторы: db, redis, cache, jobs — точка `:void.obs/instrument` (`:needs` компонентов, установка на `:after-start`, снятие на `:before-stop`, молча пропускается, когда компонента в композиции нет). Инструментации пакетов волны 2 объявлены **в самом obs**: вклад в точку, которой нет в активном множестве, — ошибка старта, поэтому `void/db` не может регистрировать свою, не сломав каждое приложение без obs (ADR-0021, уточнение к «каждый плагин регистрирует свою»); плагин, написанный после obs, контрибьютит напрямую.
- `/health` (зовёт `:health` компонентов и вклады `:void.core/health`), `/ready` (флаг готовности и состояния компонентов, без I/O — проба раз в секунду не должна ходить в базу), `/metrics` (опционально за bearer-токеном). Маршруты несут `:void.obs/endpoint`, и `void/pressure-http` не сбрасывает их наравне со своим `:void.pressure/exempt`.
- Overhead инструментации (§8.2, ≤ 7% throughput) — строка `b1-obs` в bench-suite: то же приложение B1, запущенное с obs и без.
...
### 5.25 `void/notify` — единые нотификации *(M; ADR-0040)*
Паритет Laravel Notifications; §9 прямо называет «webhook/бот-хабы» нишей, и исходящий webhook здесь — половина обещания, а не «ещё один канал».
- Пять plugin'ов в одном пакете, шов `void/cache` / `void/cache-http`: `void/notify` (точка + `:memory`/`:log`, только core), `void/notify-mail` (письмо — `void/mail`), `void/notify-inapp` (строка + колокольчик — `void/db`, `void/http`, `void/html`), `void/notify-webhook` (POST — `http/client`, `void/crypto` модульным ребром для подписи), `void/notify-jobs` (очередь — `void/jobs`).
- **Уведомление — таблица**: обязательны `:key` (имя события) и `:title` (единственная строка, которой у каждого канала есть куда лечь); `:to` — таблица адресов, ключ которой говорит, *что это за адрес* (`:email`, `:subject`, `:url`), а не какой это канал. Голая строка — сокращение для одного канала и ошибка при нескольких.
- Точка `:void.notify/channel` = `{:name :address? :project? :deliver :permanent??}`. **Две функции, а не одна**: `project` выполняется там, где вызвали (личность, локаль, CSRF-слот в реаче) и возвращает данные, `deliver` ходит в сеть — поэтому между ними помещается очередь, и повтор доставляет то же самое значение (обобщение ADR-0026 §5).
- Проекция, вернувшая nil, — `:skipped` (форма `:void.auth/deliver`); провал одного канала не уносит доставку соседнего — результат `notify/send` перечисляет `:sent`/`:queued`/`:skipped`/`:failed` по каналам, потому что «часть ушла» — нормальный исход fan-out'а.
- `[:notify :channels]` без значения — **композиция, прочитанная обратно**: все вкладённые каналы, которые куда-то доставляют; `:memory` и `:log` в это никогда не входят, и в `:prod` композиция без единого доставляющего канала не стартует.
- Очередь — `void/notify-jobs`: задача **на канал**, окончательный ответ той стороны (5xx SMTP, 4xx webhook) записывается, а не повторяется.
- In-app: одна таблица, которой владеет void, DDL приходит данными в миграцию приложения; получатель — личность из dyn `:void.auth/identity`, никогда не параметр запроса.
- Волна 6 (зависит от core, crypto, http, html, db, jobs, mail).
...
**Волна 6 — паритет и первое приложение:** void/storage (ADR-0039) → void/notify (ADR-0040) → остальное по [ROADMAP](ROADMAP.md).
