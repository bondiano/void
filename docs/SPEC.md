...
- **Client** *(волна 4, ADR-0027)*: `void/http/client` — HTTP/1.1 наружу теми же парсерами `void/http/wire`, keep-alive на одно соединение, таймаут отдельной задачей с `ev/deadline`. Без TLS (ADR-0010), без редиректов, без cookie jar, без ретраев — кроме переоткрытия keep-alive-сокета, закрытого peer'ом. Исходящий trace-контекст кладёт вызывающий (`trace/inject!`), счётчики читает `void/obs` через `client/stats`.
