...
Замеры делались дважды: на коммите 7709962 (стек волн 1.1–1.7) и на
финальном стеке v0.1 (коммит 4329268 — с lifecycle-стадиями ADR-0016,
request-id middleware и kernel/server split; access-log в B*-целях
выключен — они меряют цену ядра, цена логгера — отдельная строка ниже).
`results/baseline.jdn` заморожен на финальном стеке; в таблицах —
финальные цифры, в скобках — до-lifecycle значения.

## Результаты B0 (plaintext hello, полный router+middleware стек)

| Метрика | Бюджет §8.2 | Измерено | Вердикт |
|---|---|---|---|
| Throughput floor | ≥ 20 000 RPS | **37 220 RPS** (53 897 до lifecycle) | ✅ 1.9× запас |
| p50 @16k RPS (wrk2) | < 0.5 ms | **1.00 ms** | ❌ → бюджет скорректирован до < 2 ms |
| p99 @16k RPS (wrk2) | < 3 ms | **2.58 ms** | ✅ |

Просадка max-throughput −31% против до-lifecycle стека — цена
всегда-включённого request-id middleware (штамп + bound log context на
каждый запрос) и `:received`-штампа. Latency под целевым rate не
пострадала (p50/p99 без изменений в пределах шума). Осознанный
trade-off ADR-0016/0018: каждый запрос v0.1-приложения наблюдаем из
коробки; кандидат на оптимизацию волны 2 (пул контекст-таблиц,
дешёвый путь при пустой конфигурации sink'ов).
...
| Метрика | Бюджет §8.2 | Измерено | Вердикт |
|---|---|---|---|
| Throughput floor | ≥ 8 000 RPS | **9 642 RPS** (11 562 до lifecycle) | ✅ 1.2× запас |
| p50 @6.4k RPS (wrk2) | < 1 ms | **1.65 ms** | ❌ → бюджет скорректирован до < 2.5 ms |
| p99 @6.4k RPS (wrk2) | < 5 ms | **4.21 ms** (8.66 до lifecycle) | ❌→✅ бюджет скорректирован до < 10 ms; финальный стек проходит и старый |

Контекст класса (go-json, тот же rate 6 400): p50 **1.26 ms**, p99
**3.13 ms**, throughput 42.5k.

## Цена access-log (ADR-0018, §8.5 п. 4)

Строка «B* с middleware = −X%» для встроенного request-id + access-log
(полная методика, `[:http :access-log]`):

| Цель | Без access-log | С access-log | Дельта |
|---|---|---|---|
| B0 max throughput | 37 220 RPS | 28 548 RPS | **−23%** |
| B1 max throughput | 9 642 RPS | 9 134 RPS | **−5%** |
| B1 p99 @6.4k | 4.21 ms | 24.25 ms | хвост от записи в sink |

Поэтому B*-цели меряют ядро (access-log выключен в их конфиге), а
приложения получают наблюдаемость по умолчанию и ручку
`[:http :access-log] false` там, где RPS дороже логов. Хвост B1 p99 с
логом — цена синхронной части jdn-sink'а на этой частоте записей;
батчинг writer-fiber'а уже смягчает её, дальнейшее — волна 2/3 (obs).
...
5. Штамп `:commit` в baseline.jdn — 4329268: финальный стек v0.1 с
   lifecycle-стадиями. Ранний замер (7709962, до lifecycle) сохранён
   в таблицах выше как контекст.
