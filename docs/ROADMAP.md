...
1. Demo: приложение с логином (password + magic link), row-level ABAC, audit через bus, полные метрики/трейсы. **Наполовину закрыт в 3.4**: `examples/blog` вошёл в волну 3 — вход по паролю на сессии, регистрация, row-level политика «автор правит свой пост» (та же политика, которой enforcement закрывает маршрут, решает, рисовать ли кнопку Edit), CSRF без единой строки в приложении и `[:authz :default :deny]`, при котором маршрут без политики не стартует; `test/auth-test.janet` гоняет это на обоих движках. Остаются magic link (нужен 3.5 `void/mail` как доставка) и audit через bus (3.6).
