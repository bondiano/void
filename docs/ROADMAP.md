...
| Волна 3, остальное | 3.6 `void/bus` — не начат; exit-критерии 2 (outbox) и 3 (бюджеты) вместе с ним |
...
1. Demo: приложение с логином (password + magic link), row-level ABAC, audit через bus, полные метрики/трейсы. **Закрыт, кроме audit'а**: `examples/blog` вошёл в волну 3 — вход по паролю на сессии, регистрация, row-level политика «автор правит свой пост» (та же политика, которой enforcement закрывает маршрут, решает, рисовать ли кнопку Edit), CSRF без единой строки в приложении и `[:authz :default :deny]`, при котором маршрут без политики не стартует. **Magic link добавлен в 3.5**: `auth/challenge!` — весь код приложения, письмо принадлежит `void/mail-auth`, а уезжает оно **через ту же очередь**, в которой крутится счётчик комментариев, и ни один обработчик об этом не знает; страница отвечает одинаково на адрес с аккаунтом и без него. `test/auth-test.janet` гоняет это на обоих движках. Остаётся audit через bus (3.6).
