...
- [ ] Установочный шов: `jpm --tree=… install` из корня + `JANET_PATH` — локальный аналог CI-джобы; в `scripts/packages.janet` пример помечен как тестируемый от установки, и `packages/check` не требует от него checkout-путей. Цена (правка фреймворка = переустановка дерева) написана в README примера, а не оставлена читателю на открытие
- [ ] Первый коммит — ровно то, что выдали `void new` и `void make auth`, без единой ручной правки: дальше каждая правка сгенерированного видна в git, и каждая — кандидат в задачу волны
- [ ] Приём: `X-Hub-Signature-256` через `crypto/hmac-sha256` и `crypto/equal?` (не `=`), идемпотентность по `X-GitHub-Delivery`, сырое тело в `void/storage`, метаданные в БД, `202` до всякой маршрутизации; приём под `void/pressure`, потому что GitHub шлёт всплесками — и это строка примера в bench-таблице (§8.5)
- [ ] Правила маршрутизации — данные (`{:when {:event … :repo …} :to [:telegram]}`), матчинг — чистая функция с таблицей примеров в тесте, а не `cond` по событиям в контроллере
- [ ] Telegram — вклад приложения в `:void.notify/channel`, ровно та пара из [ADR-0040](adr/0040-notifikacii-odno-sobytie-neskolko-kanalov.md): `:project` строит текст без сети, `:deliver` ходит через `http/client` + `void/tls`, `:permanent?` записывает 400/403 и повторяет 429 (`retry_after`) и 5xx; отправка через `void/notify-jobs`, задача на канал. Первая внешняя проверка обещания «telegram — приложением или пакетом по спросу»
- [ ] Эксплуатация: jobs-дашборд 6.3 — главный экран, а не демо; страница доставок с сырым телом по signed URL и `void hub replay <delivery>`, который заодно снимает нужду в туннеле на разработке: одна настоящая доставка воспроизводится сколько угодно раз
