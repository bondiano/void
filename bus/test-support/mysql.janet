# Is there a MySQL to test void/bus-db against?
#
# The third engine the backend claims to run on, and the one with the
# most to say back: no ON CONFLICT, no partial indexes, no `CREATE
# INDEX IF NOT EXISTS`, a TEXT column that cannot be a key — every one
# of them a branch in void/bus/db that only a real server exercises.
