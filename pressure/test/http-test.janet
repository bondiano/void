...
(assert (= 3 (get-in report [:extensions :void.pressure/check :contributions]))
        "and the check the app contributed is in the point, next to the two built-in pool ones")
...
(defer (plugin/shutdown! boot 3)
  ...
  (assert (= 3 (length (st :checks)))
          "with the contributed check wired in next to the two built-in pool ones")
